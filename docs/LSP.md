...
- **Request `snaqlite/graph/fetchResultSlice`** — Fetches a paginated slice of a result at an optional **path** (for nested vectors/maps).  
  ...
  - **Response:** `elements` (array of slice elements), `totalCount` (number), `hasMore` (boolean), optional `nextCursor` (string).  
  ...
  - Path semantics: **0-based** indices for vectors (e.g. `[5]` = 6th element). Map entries are sliced in **registration order**. For replayable lazy vectors, slices are computed from a streaming window and avoid full vector materialization when vector length is structurally known. For non-replayable stream-backed vectors (`FromInput`) under `resultHandle`, the contract is **forward-only**: first page must start at `offset = 0`; later pages require `cursor` continuation. Random jumps without cursor are rejected with `invalid_params`. If the widget is not found or the path is invalid, the server returns an error.
...
### Canvas isolation (hard binding + rebind)
...
Optional helpers for UI ergonomics while keeping source text canonical:
...
- **Non-draining progress/display paths** — Summary payload, progress notifications, and formatting paths must not consume forward-only (`FromInput`) receivers; forward-only consumption is owned by handle-based slice paging.
