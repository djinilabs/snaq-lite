...
fn collect_bound_names(def: &ExprDef) -> HashSet<String> {
    ...
    fn go(def: &ExprDef, names: &mut HashSet<String>) {
        match def {
            ...
            | ExprDef::LitTemporal(_) | ExprDef::LitDate(_) | ExprDef::ExternalStream(_) | ExprDef::InputDecl(_, _, _)
...
fn collect_bound_names_spanned(def: &SpannedExprDef) -> HashSet<String> {
    ...
    fn go(def: &SpannedExprDef, names: &mut HashSet<String>) {
        match &def.value {
            ...
            | SpannedExprDefKind::ExternalStream(_) | SpannedExprDefKind::InputDecl(_, _, _)
...
fn substitute_symbols_inner(
    ...
    match def {
        ...
        ExprDef::InputDecl(name, param_id, type_name) => Ok(ExprDef::InputDecl(
            name.clone(),
            param_id.clone(),
            type_name.clone(),
        )),
