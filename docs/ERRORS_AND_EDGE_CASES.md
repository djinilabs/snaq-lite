...
| **CLI --stream-variance** | The option `--stream-variance` accepts only **zero** or **infer**. Any other value (e.g. `exact`) causes the CLI to exit with **error: --stream-variance must be zero or infer, got "…"**. See [EXTERNAL_STREAMS.md](EXTERNAL_STREAMS.md#limits-and-edge-cases-cli). |
