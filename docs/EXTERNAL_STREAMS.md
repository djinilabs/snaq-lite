...
- **Order-statistics reducers:** Exact reducers that require global ordering (`median`, `quantile`) are materializing operations and reject non-replayable stream inputs unless materialized upstream. Streaming alternatives (`median_approx`, `quantile_approx(p)`) work on forward-only streams.
