...
- **Order-statistics reducers:** Exact reducers that require global ordering (`median`, `quantile`) are materializing operations (even for replayable streams) and reject non-replayable stream inputs unless materialized upstream. Streaming alternatives (`median_approx`, `quantile_approx(p)`) work on forward-only streams.
- **Forward-only ownership:** For non-replayable stream handles, forward-only consumption is owned by explicit paging/consumption paths. Summary/display/progress helpers should not drain these streams.
- **Incremental runtime sessions:** For repeated evaluations where you want dependency-driven invalidation across updates, use the long-lived `Session` API (`Session::new`, `Session::eval`, `Session::eval_with_stream_inputs`) instead of rebuilding a fresh runtime each call.
- **Errors (WASM):** Unsupported value types in expressions (e.g. map or undefined in an if-blend, or map in arithmetic like `$x * 10` when `$x` is CSV rows) now produce a **runtime error** (e.g. `IfBranchTypeMismatch`, `UnsupportedVectorOperation`) instead of panicking. The LSP and widget layer surface these as diagnostics and Error status, so the worker keeps running and the user sees a clear error message.
...
In the graph UI, **file blocks** are output-only nodes that hold a URL (e.g. a dropped file as a blob URL, or `indexeddb://projectId/nodeId` for persisted files). When you drop a file onto the canvas, its **original file name** is stored and shown in the block title so you can tell multiple file blocks apart (e.g. "sales.csv", "readings.json"). When a file block is connected to a computation or presentation input, the frontend builds a map from input name to stream index by calling `createStreamInput`, then **feeding** the stream in the worker so that parsing runs in WASM with back-pressure:
